...
public class Images
	{
		...
		public BufferedImage[] boundaries;
		public BufferedImage[] icons2;

		// Gui
		public BufferedImage gui;
		public BufferedImage[] infoPop;

		// Menu's
		public BufferedImage[] menuButtons;
		public BufferedImage[] helpSlides;
		public BufferedImage begin;
		...
		private final void loadImages() throws IOException
			{
				BufferedImage tileSheet = ImageIO.read(Images.class.getResource(imageDir + "/tiles.png"));
				BufferedImage boundarySheet = ImageIO.read(Images.class.getResource(imageDir + "/Boundaries.png"));
				BufferedImage menuButtonSheet = ImageIO.read(Images.class.getResource(imageDir + "/menuButtons.png"));
				BufferedImage iconsSheet = ImageIO.read(Images.class.getResource(imageDir + "/icons.png"));
				BufferedImage helpSheet = ImageIO.read(Images.class.getResource(imageDir + "/help.png"));
				BufferedImage infoPopSheet = ImageIO.read(Images.class.getResource(imageDir + "/infoPop.png"));

				gui = ImageIO.read(Images.class.getResource(imageDir + "/GUI.jpeg"));
				begin = ImageIO.read(Images.class.getResource(imageDir + "/begin.png"));

				// TODO create proper infoPop images
				infoPop = new BufferedImage[3];
				for (int i = 0; i < 3; i++)
					infoPop[i] = getCroppedImage(infoPopSheet, i * 40, 0, 40, 40);

				menuButtons = new BufferedImage[3];
				for (int i = 0; i < 3; i++)
					menuButtons[i] = getCroppedImage(menuButtonSheet, 0, i * 200, 800, 200);

				helpSlides = new BufferedImage[8];
				for (int i = 0; i < 8; i++)
					helpSlides[i] = getCroppedImage(helpSheet, i * 200, 0, 200, 200);

				tiles = new BufferedImage[19][6];
				for (int i = 0; i < 19; i++)
					for (int j = 0; j < 6; j++)
						tiles[i][j] = getCroppedImage(tileSheet, i * 40, j * 40, 40, 40);

				icons2 = new BufferedImage[12];
				for (int i = 0; i < 12; i++)
					icons2[i] = getCroppedImage(iconsSheet, i * 40, 0, 40, 40);

				boundaries = new BufferedImage[6];
				for (int i = 0; i < 6; i++)
					boundaries[i] = getCroppedImage(boundarySheet, i * 40, 0, 40, 40);
			}
